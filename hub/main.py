...
async def save_processed_agent_data(processed_agent_data: ProcessedAgentData):
    ...
    if redis_client.llen("processed_agent_data") >= BATCH_SIZE:
        ...
        for _ in range(BATCH_SIZE):
            popped_data = redis_client.lpop("processed_agent_data")
            processed_agent_data = ProcessedAgentData.model_validate_json(popped_data)
            processed_agent_data_batch.append(processed_agent_data)
        print(processed_agent_data_batch)
        store_adapter.save_data(processed_agent_data_batch=processed_agent_data_batch)
        send_message(client, MQTT_TOPIC, processed_agent_data_batch)
        batch = True
...
def send_message(client, topic, messages):
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        processed_agent_data_batch: List[ProcessedAgentData] = []
        if redis_client.llen("processed_agent_data") >= BATCH_SIZE and not batch:
            for _ in range(BATCH_SIZE):
                processed_agent_data = ProcessedAgentData.model_validate_json(
